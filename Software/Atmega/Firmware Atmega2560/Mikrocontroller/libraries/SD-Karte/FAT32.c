...
while(1)
{
     ...
     for(i=0; i<bytesPerSector; i+=32)
     {
        ...
        if(dir->name[0] == EMPTY) //indicates end of the file list of the directory
		{
		  if((flag == GET_FILE) || (flag == DELETE))
		      transmitString_F((char*)PSTR("File does not exist!"));
		  return 0;   
		}
		if((dir->name[0] != DELETED) && (dir->attrib != ATTR_LONG_NAME))
        {
          if((flag == GET_FILE) || (flag == DELETE))
          {
            ...
            if(j == 11)
			{
			  ...
			  {
			     TX_NEWLINE;
				 transmitString_F((char*)PSTR("Deleting.."));
				 TX_NEWLINE;
				 TX_NEWLINE;
			     ...
			     while(1)  
			     {
					...
					if(nextCluster > 0x0ffffff6) 
					   {transmitString_F((char*)PSTR("File deleted!"));return 0;}
          ...
          else  //when flag = GET_LIST
		  {
			 ...
			 {
			     transmitString_F ((char*)PSTR("FILE" ));
		         transmitString_F ((char*)PSTR("   "));
			     displayMemory (LOW, dir->fileSize);
			 }
			 else
			   transmitString_F ((char*)((dir->attrib == 0x10)? PSTR("DIR") : PSTR("ROOT")));
		  }
       }
     }
   }
   ...
   if(cluster > 0x0ffffff6)
   	 return 0;
   if(cluster == 0) 
   {transmitString_F((char*)PSTR("Error in getting cluster"));  return 0;}
 }
return 0;
}
...
while(1)
{
  ...
  {
	  //liest Block von 512 Bytes aus mit SPI und schreibt Werte in buffer[]
  ...
  cluster = getSetNextCluster (cluster, GET, 0);
  if(cluster == 0) {transmitString_F((char*)PSTR("Error in getting cluster")); return 0;}
}
return 0;
}
...
if(j>8) {transmitString_F((char*)PSTR("Invalid fileName..")); return 1;}
...
if(j == 1) 
{
  transmitString_F((char*)PSTR("  File already existing, appending data..")); 
  appendFile = 1;
...
{
  TX_NEWLINE;
  transmitString_F((char*)PSTR(" Creating File.."));
  ...
  cluster = searchNextFreeCluster(cluster);
   ...
   {
      TX_NEWLINE;
      transmitString_F((char*)PSTR(" No free cluster!"));
	  return;
   }
...
while(1)
{
   ...
   TX_NEWLINE;
   transmitString_F((char*)PSTR(" Enter text (end with ~):"));
...
if(appendFile)  //executes this loop if file is to be appended
{
  ...
  TX_NEWLINE;
  transmitString_F((char*)PSTR(" File appended!"));
  TX_NEWLINE;
  return;
}
...
TX_NEWLINE;
TX_NEWLINE;
transmitString_F((char*)PSTR("Total Memory: "));
...
TX_NEWLINE;
transmitString_F((char*)PSTR(" Free Memory: "));
